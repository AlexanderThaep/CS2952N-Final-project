...
def main() -> None:
    ...
    sample = sample.cuda().to(torch.float32)

    torch.cuda.empty_cache()
    output = extractor.extract(sample_id="", sample=sample)

    print(f"Final: {output.layer_name} with latents shape: {output.layer_latent.shape}") 
